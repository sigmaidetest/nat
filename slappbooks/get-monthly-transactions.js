...
exports.handler = function (event, context, callback) {
	...
	rds.query({
		...
		if (error) {
			...
			let y = [entityName, year.concat("-").concat(month).concat("-01"), year.concat("-").concat(month).concat("-31"), startIndex, pageSize];
			...
			rds.query({
				...
				if (error) {
					...
					if (startIndex == 0) {
						...
						let z = [entityName, year.concat("-").concat(month).concat("-01")];
						...
						rds.query({
							...
							if (error) {
								...
								let k = [entityName, year.concat("-").concat(month).concat("-01")];
